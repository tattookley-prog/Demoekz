...
echo
echo "--- Сеть HQ-SRV ---"
read -rp "IP/маска HQ-SRV [192.168.1.2/27]: " SRV_IP_CIDR
SRV_IP_CIDR="${SRV_IP_CIDR:-192.168.1.2/27}"

read -rp "Шлюз HQ-SRV [192.168.1.1]: " SRV_GW
SRV_GW="${SRV_GW:-192.168.1.1}"
...
IP_BR_SRV="${IP_BR_SRV:-192.168.3.2}"
...
if [[ -z "$SSHD_CONF" ]]; then
    ...
    for svc in sshd ssh; do
        if systemctl enable "$svc" 2>/dev/null && systemctl restart "$svc" 2>/dev/null; then
            ok "$svc перезапущен (порт ${SSH_PORT})"
            SSH_STARTED=1
            break
        fi
    done
    if [[ $SSH_STARTED -eq 0 ]]; then
        if service sshd restart 2>/dev/null; then
            ...
            ok "sshd перезапущен через service (порт ${SSH_PORT})"
            SSH_STARTED=1
        fi
    fi
...
ptr-record=$(echo "$IP_HQ_RTR" | awk -F. '{print $4"."$3"."$2"."$1}').in-addr.arpa,hq-rtr.${DOMAIN}
ptr-record=$(echo "$IP_HQ_SRV" | awk -F. '{print $4"."$3"."$2"."$1}').in-addr.arpa,hq-srv.${DOMAIN}
ptr-record=$(echo "$IP_HQ_CLI" | awk -F. '{print $4"."$3"."$2"."$1}').in-addr.arpa,hq-cli.${DOMAIN}
EOF2
...
EOF2
        ...
        sleep 1
        RESOLVED=$(getent hosts "hq-srv.${DOMAIN}" 2>/dev/null | awk '{print $1}' || true)
        if [[ "$RESOLVED" == "$IP_HQ_SRV" ]]; then
            ok "DNS отвечает: hq-srv.${DOMAIN} → ${IP_HQ_SRV}"
        else
            warn "DNS проверка: ожидалось ${IP_HQ_SRV}, получено '${RESOLVED}' — проверьте вручную"
        fi
        STATUS["dns"]="OK"
    else
